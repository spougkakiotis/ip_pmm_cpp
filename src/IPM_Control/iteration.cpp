...
IterStatus ip_pmm_iteration(SolverState& s,
    ...
    const Complementarity comp0 = compute_complementarity(qp, s.x, s.z_l, s.z_u);
    if (comp0.n_bounds > 0) {
        s.mu = boundary_control(qp, s.x, s.z_l, s.z_u, s.mu);
    }

    // Non-regularized residuals (also fed to the loop for stopping / PMM update).
    const std::vector<Scalar> nr_res_p = primal_residual(qp, s.x);
    const std::vector<Scalar> nr_res_d = dual_residual(qp, s.x, s.y, s.z_l, s.z_u);
    s.nr_res_p_norm = norm(nr_res_p);   // 2-norm (loop will also use inf-norm for stopping)
    s.nr_res_d_norm = norm(nr_res_d);
